...
Generate responses for each condition: python src/normal.py, python src/top1.py, python src/wrong.py (misrouted condition)
Compute semantic entropy across all three conditions: python src/SemanticEntropy.py
Run statistical tests: python src/statTest.py
Generate visualisations: python src/visualisations.py

Data Format
data/prompts.jsonl contains one prompt per line in the following format: {"category": "factual", "prompt": "Your prompt here"}
